    ...
    <div class="main">
      <ul class="fx" style="align-items: center;">
        ...
        <li class="list" :class="disembark ? '' : 'pointer'">
          <span
            @click="toPersonalCenter(disembark ? 'none' : 'toLogin')"
          >{{disembark ? nickName : '登录/注册'}}</span>
        ...
        <li class="list pointer" style="position:relative;">
          <span
            v-if="inSchool < 1 || inSchool === 2"
            @click="toPage('/schoolEnter')"
            @mouseenter="updateUserInfoData"
          >学校注册/入驻</span>
          <el-dropdown v-else @command="toPage">
            ...
            <el-dropdown-menu v-if="inSchool === 1" slot="dropdown" class="in-school">
              <el-dropdown-item
                command="/"
                style="line-height: 20px;padding: 10px 16px 20px;"
              >系统正在加急审核机构信息中，请耐心等待，为您带来的不便，请敬请谅解！！如有问题请致电客服。</el-dropdown-item>
            </el-dropdown-menu>
            ...
            <el-dropdown-menu v-else slot="dropdown" class="in-school">
              <el-dropdown-item
                command="/"
                style="line-height: 20px;padding: 10px 16px 20px;"
              >由于您操作违反了用户协议，学校权限暂被关闭，为您带来的不便，请敬请谅解！如有问题请致电客服。</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </li>
            ...
            <el-dropdown-menu v-else slot="dropdown" class="el-dropdown-menu-left">
              <el-dropdown-item class="fx" command="updateLoad">
                <img
                  width="32px"
                  height="32px"
                  :src="this.$store.getters.headPortraitUrl(this)"
                  alt
                />
                <span>{{nickName}}</span>
              </el-dropdown-item>
        ...
        <!-- 消息导航 -->
        <li class="list pointer">
          <el-dropdown hide-on-click @visible-change="isShow" @command="toPage" :show-timeout="100">
            <div class="el-dropdown-link">
            ...
            <el-dropdown-menu v-if="!disembark" slot="dropdown" class="disembark2">
              <el-dropdown-item>
                <span @click="toPersonalCenter('quit')">请登录</span>
              </el-dropdown-item>
            </el-dropdown-menu>
            ...
            <el-dropdown-menu v-else slot="dropdown" class="el-dropdown-menu-left1 fx">
              ...
              >
                ...
                <div
                  ...
                  :key="i"
                  class="pointer fx main_message"
                  @click="toMsgPage(item.name, item.id)"
                >
              ...
              <el-dropdown-item
                divided
                class="el-dropdown-menu-left1-footer"
  ...
  methods: {
    ...
    toPage(url) {
      ...
      if (url === "schoolAdmin") {
        //跳转到学校后台
        location.href =
          "http://192.168.3.87:8081/#/index?schoolNum=" +
          this.compile(userInfo.user.phone);
        return;
      }

      if (
        this.$router.history.current.path === "/systemMessage" &&
        url === "/systemMessage"
      )
        return;
      let query = {};
    ...
    async toMsgPage(val, id) {
      ...
      if (
        current.path === "/systemMessage" &&
        current.query.category * 1 === query.category
      ) {
