  ...
  <div class="msg fx">
    <div class="wrap-1200 main fx">
      <div class="left-nav col999">
        <span
          class="fx fxCenter pointer"
          :class="(cutBool !==2 && cutBool !==3) ? 'bgc-fff col333' : ''"
          @click="cut(1)"
        >系统</span>
        <span
          v-if="(isSchool && isService) || !isSchool"
          class="fx fxCenter pointer"
          :class="cutBool === 2 ? 'bgc-fff col333' : ''"
          @click="cut(2)"
        >客服</span>
        <span
          class="fx fxCenter pointer"
          :class="cutBool === 3 ? 'bgc-fff col333' : ''"
          @click="cut(3)"
        >学校</span>
      </div>
      <messageList
        ...
        :oldCurrentPage3.sync="oldCurrentPage3"
      ></messageList>
...
export default {
  ...
  methods: {
    ...
    changeChat(userId, schoolId) {
      ...
      this.push({
        path: rt.path,
        query: { category: this.cutBool, id: userId }
      });
    ...
    getIsService(currentPage = null, pageSize = null) {
      ...
      return new Promise(resolve => {
        let schoolId =  currentPage === null ? "" : userInfo.schoolUser ? userInfo.schoolUser.id : "";
